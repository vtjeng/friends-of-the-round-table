  ...
  for (int i = 0; i < n - 1; ++i) {
    for (int j = i + 1; j < n; ++j) {
      ...
      for (const auto &guest_pair : newly_adjacent_guest_pairs) {
        const int idx =
            pair_to_index(std::get<0>(guest_pair), std::get<1>(guest_pair));
        ++adjacency_count[idx];
      }
      ...
      if (num_remaining_switches > 1) {
        ...
        const std::optional<std::vector<std::tuple<int, int>>> r =
...
int main() {
  // TODO: `n` and `num_switches` should probably be accepted via stdin, not
  // hardcoded here
  int n = 7;
  int num_switches = 4;

  std::optional<std::vector<std::tuple<int, int>>> v =
